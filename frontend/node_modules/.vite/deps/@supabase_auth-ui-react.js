      ...
      for (c2 in e5) {
        ...
        for (d2 of z2) {
          ...
          if (x2 && (u3 = c2.slice(1) in r2.media ? "@media " + r2.media[c2.slice(1)] : c2, c2 = u3.replace(/\(\s*([\w-]+)\s*(=|<|<=|>|>=)\s*([\w-]+)\s*(?:(<|<=|>|>=)\s*([\w-]+)\s*)?\)/g, (e7, t4, n4, r3, i3, o3) => {
            ...
            void 0 === o2 && (o2 = [[], t3, n3]), c2 = x2 || 36 !== c2.charCodeAt(0) ? c2 : `--${k(r2.prefix)}${c2.slice(1).replace(/\$/g, "-")}`, d2 = z3 ? d2 : "number" == typeof d2 ? d2 && e6 in R ? String(d2) + "px" : String(d2) : y(b(e6, null == d2 ? "" : d2), r2.prefix, r2.themeMap[e6]), o2[0].push(`${x2 ? `${c2} ` : `${g(c2)}:`}${d2}`);
        }
      }
      var u3, S3;
    };
  ...
  for (const t3 of n2)
    if (null != t3)
      if (t3[s]) {
        ...
        for (const e4 of t3[s].composers)
          r2.composers.add(e4);
      } else
        t3.constructor !== Object || t3.$$typeof ? null == r2.type && (r2.type = t3) : r2.composers.add(P(t3, e3));
...
var L = (e3, t2, n2) => {
  const [r2, i2, o2, l2] = O(t2.composers), c2 = "function" == typeof t2.type || t2.type.$$typeof ? ((e4) => {
    ...
    for (const e4 of String(s2.className || "").trim().split(/\s+/))
      e4 && h2.add(e4);
      ...
      if (!t2.rules.global.cache.has(n3)) {
        if (t2.rules.global.cache.add(n3), "@import" in r3) {
          ...
          for (let n4 of [].concat(r3["@import"]))
            n4 = n4.includes('"') || n4.includes("'") ? n4 : `"${n4}"`, t2.sheet.insertRule(`@import ${n4};`, e4++);
          delete r3["@import"];
        }
        x(r3, [], [], e3, (e4) => {
